  ...
  (mapping
   (kv "schema"
       (mapping
        ...
        (kv "description" "朙月拼音＋小鶴雙拼方案，使用 rime-ice 詞庫。")
        (kv "dependencies" (sequence "cangjie6"))))
   (kv "switches"
       (sequence
        (mapping (kv "name" "ascii_mode")
                 (kv "reset" 0)
                 (kv "states" (sequence "鶴" "A")))
   ...
   (kv "speller"
       (mapping
        (kv "alphabet" "zyxwvutsrqponmlkjihgfedcba")
        (kv "delimiter" " '")
        (kv "algebra"
            (sequence
             "erase/^xx$/"
   ...
   (kv "translator"
       (mapping
        ...
        (kv "prism" "flypy_ice")
        (kv "preedit_format" (mapping (kv "__include" "flypy:/flypy")))))
   (kv "reverse_lookup"
       (mapping
